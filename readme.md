...
- Простая и понятная в применении.
- Код легко переиспользовать и расширить под свои нужды.
- Не имеет никаких зависимостей (нет зависимого кода, библиотек).
- Включает специальные настройки и функции. Поставляется с SSL-сертификатами от Mozilla. 
...
### Устанавливаем свой User-Agent и убеждаемся, что именно он был отправлен (GET-запрос) 
```php
...
- Внести изменения в код, который вы собираетесь отправить в основной репозиторий. Если вы создали **новый метод**, то добавьте
  в PHPDoc свой тэг `author` в точности по примеру, как сделано в существующем коде (также такой тэг нужно добавить в конец списка
